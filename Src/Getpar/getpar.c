 ...
 *  ---------------------
 *   6    14    Bruce Worden    Added type "z" for size_t variables (not lists)
 *   3    94    Blair Zajac     Changed (looks like he ANSIfied it -- CBW)
	...
	switch(*type)
	   {
		...
		case 'z': typemess= "an unsigned integer";	break;
			...
			case 'z':
				...
				if (dumm < 0) {
					gp_getpar_err("getpar",
					  "negative value for size_t variable '%s'", name);
				} else {
			        	sizt = (size_t *) val;
					*sizt = dumm;
					found=1;
		...
		switch(*type)
		   {
			case 'd': sprintf(line,"(int) = %d",*( (int *) val));
				break;
			case 'z': sprintf(line,"(size_t) = %zu",*( (size_t *) val));
				break;
